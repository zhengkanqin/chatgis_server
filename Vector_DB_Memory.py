...
class VectorDBMemory(Memory):
    ...
    async def query(self, query: str | MemoryContent, metadata_filter: dict = None, cancellation_token=None,
                    **kwargs) -> MemoryQueryResult:
        """
        查询相关内容，支持语义搜索和元数据过滤
    ...
    async def list_modified_data(self) -> dict:
        ...
        Returns:
            dict: 包含以下信息：
                ...
                每个数据项包含：
                    ...
                    - metadata: 元数据
                    - file_path: 文件路径
                    - last_modified: 最后修改时间（如果文件存在）
                    - status: 文件状态（"modified"或"deleted"）
        """
        import os
        from datetime import datetime
        ...
        # 按最后修改时间倒序排序（文件不存在的放在最后）
        modified_data.sort(key=lambda x: (x['last_modified'] is None, x['last_modified'] or ''), reverse=True)
