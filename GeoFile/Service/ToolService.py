...
#
#     2. 属性查询 - 灵活查询SHP文件属性数据：
#        - 要素字段查询(单个或多个要素字段): operation='query', params={'query_target': 'single', 'target_ids': [要素ID], 'attributes': ['字段名']}
#        - 横向查询(单行或多行): operation='query', params={'query_target': 'row', 'target_ids': [要素ID]}
#        - 纵向查询(单列或多列): operation='query', params={'query_target': 'column', 'attributes': ['字段名']}
#        - 全表查询: operation='query', params={'query_target': 'all'}
#
...
#     params = params or {}
#
#     # 根据operation类型过滤不需要的参数
#     if operation == "convert":
#         # 需要attributes/output_path
#         filtered_params = {k: v for k, v in params.items() if k in ["attributes", "output_path"]}
#     elif operation == "query":
#         # 需要attributes和query_target，target_id仅在query_target='row'时需要
...
async def to_geojson(file_path: str,
    ...
    - output_path: 缓冲区输出目录路径(可选，若没有则输出到默认路径)
